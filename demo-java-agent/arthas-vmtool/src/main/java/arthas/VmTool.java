...
public class VmTool implements VmToolMXBean {
    ...
    @Override
    public <T> T getSingleInstance(Class<T> klass) {
        T[] instances = getInstances(klass);
        if (instances.length > 1) {
            throw new RuntimeException("more than one instance exists in jvm,klass:" + klass.getName());
        }
        return instances[0];
    }
